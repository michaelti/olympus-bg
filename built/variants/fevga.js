...
const Fevga = () => ({
    ...
    isMoveValid(from, to) {
        ...
        if (to === 25 || to === 0) {
            if (to === 25)
                to = 12;
            if (this.turn === Player.white && (from > 18 || from < 13))
                return false;
            ...
            if (!this.dice.includes(pipDistance(from, to))) {
                ...
                if (this.dice[0] > pipDistance(from, to) || this.dice[1] > pipDistance(from, to)) {
                    ...
                    const farHomePips = this.turn === Player.white ? range(from + 1, 18) : range(from + 1, 6);
        ...
        // Standard move
            ...
            if (this.pips[to].top !== this.turn && this.pips[to].size > 0)
                return false;
            if (this.turn === Player.white && from >= 13 && to <= 12)
                return false;
            if (this.turn === Player.black && from <= 12 && to >= 13)
                return false;
    ...
    doMove(from, to) {
        ...
        if (this.state[this.turn] === State.start)
            ...
            if ((this.turn === Player.white && to > 12) ||
